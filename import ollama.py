...
for i in range(NUM_TEST):
    ...
    # ── NUOVO: Lista per il vettore delle distribuzioni (T, F) ──
    ...
    for rep in range(RIPETIZIONI_PER_DOMANDA):
        ...
        if not primo_successo:
            ...
            if   "true"  in risposta_pulita: risposta_ufficiale = "true"
            elif "false" in risposta_pulita: risposta_ufficiale = "false"
            else:                             risposta_ufficiale = "errore_modello"
            primo_successo = True

        if   "true"  in risposta_pulita: conteggi_ripetizioni["true"]  += 1
        elif "false" in risposta_pulita: conteggi_ripetizioni["false"] += 1
        else:                             conteggi_ripetizioni["errore"] += 1

        print(f"  Ripetizione {rep+1}: {risposta_pulita}")

        if   "true"  in risposta_pulita: running_true  += 1
        elif "false" in risposta_pulita: running_false += 1
        ...
        # ── Estrazione logprobs singola ripetizione per media totale ──
        p_t_rep = 0.0
        p_f_rep = 0.0
        if 'logprobs' in resp_json and isinstance(resp_json.get('logprobs'), list) and len(resp_json['logprobs']) > 0:
            ...
            for token_dict in top_probs_dict:
                ...
                if   "true"  in token_text: p_t_rep += math.exp(logp)
                elif "false" in token_text: p_f_rep += math.exp(logp)

            somma_rep = p_t_rep + p_f_rep
            if somma_rep > 0:
                # ── NUOVO: Calcolo e salvataggio probabilità normalizzata singola ──
    ...
    # ── NUOVO: Stampa a terminale il vettore con le 10 distribuzioni ──
    ...
    prob_true_raw = 0.0
    prob_false_raw = 0.0
    if 'logprobs' in resp_json_principale and isinstance(resp_json_principale.get('logprobs'), list) and len(resp_json_principale['logprobs']) > 0:
        primo_token_data = resp_json_principale['logprobs'][0] if isinstance(resp_json_principale['logprobs'], list) else {}
        top_probs_dict = primo_token_data.get('top_logprobs', [])
        for token_dict in top_probs_dict:
            token_text = token_dict.get('token', '').strip().lower()
            logp = token_dict.get('logprob', -100)
            if   "true"  in token_text: prob_true_raw  += math.exp(logp)
            elif "false" in token_text: prob_false_raw += math.exp(logp)
...
def mostra_interfaccia_completa():
    finestra = tk.Tk()
    finestra.title(f"Report Benchmark ({RIPETIZIONI_PER_DOMANDA} Ripetizioni)")
    ...
    tabella.heading("Domanda",       text="Inizio Domanda");        tabella.column("Domanda",       width=160)
    tabella.heading("ProbMedia",     text="Media Prob. [10 Rip]");  tabella.column("ProbMedia",     width=135, anchor="center")
    tabella.heading("Distribuzione", text="Dist. [Rip.]");          tabella.column("Distribuzione", width=70,  anchor="center")
    tabella.heading("Reale",         text="Reale");                  tabella.column("Reale",         width=50,  anchor="center")
    tabella.heading("H_min",         text="H min");                  tabella.column("H_min",         width=60,  anchor="center")
    tabella.heading("H_max",         text="H max");                  tabella.column("H_max",         width=60,  anchor="center")
    tabella.heading("H_media",       text="H media (bit)");          tabella.column("H_media",       width=90,  anchor="center")
    ...
    # ==========================================
    # SCHEDA 2: Grafico Box Plot — ENTROPIA
    # ==========================================
    tab2 = ttk.Frame(notebook)
    notebook.add(tab2, text="Grafico Entropia (bit)")
    ...
    for val in entropia_corrette:
        dati_plot.append(val); etichette.append("Corrette")
    for val in entropia_errate:
        dati_plot.append(val); etichette.append("Errate")
    for val in entropia_corrette + entropia_errate:
        dati_plot.append(val); etichette.append("Aggregato")

    if dati_plot:
        sns.boxplot(
            x=etichette, y=dati_plot, hue=etichette,
            palette={"Corrette": "#c6efce", "Errate": "#ffc7ce", "Aggregato": "#222222"},
    ...
    # ==========================================
    # SCHEDA 3: Istogramma [Conteggio]
    # ==========================================
    tab3 = ttk.Frame(notebook)
    notebook.add(tab3, text="Statistiche Ripetizioni")
    ...
    valori_y = [distribuzioni_totali.get(cat, 0) for cat in categorie_x]

    fig3 = Figure(figsize=(6, 4), dpi=100)
    ax3  = fig3.add_subplot(111)
    sns.barplot(x=categorie_x, y=valori_y, hue=categorie_x, ax=ax3, palette="viridis", legend=False)
    ax3.set_title(f"Frequenza Combinazioni su {RIPETIZIONI_PER_DOMANDA} Ripetizioni")
    ax3.set_xlabel("Combinazione (Maggioranza - Minoranza)")
    ax3.set_ylabel("Numero di Domande")
    ax3.set_ylim(0, NUM_TEST)
    ax3.yaxis.set_major_locator(MultipleLocator(5))
    ax3.yaxis.set_minor_locator(MultipleLocator(1))
    ax3.grid(axis='y', which='major', linestyle='-',  linewidth=0.8, alpha=0.7)
    ax3.grid(axis='y', which='minor', linestyle='--', alpha=0.3)
    ...
    notebook.add(tab4, text="Statistiche Ripetizioni [%]")

    valori_y_perc = [(val / NUM_TEST) * 100 for val in valori_y]
    fig4 = Figure(figsize=(6, 4), dpi=100)
    ax4  = fig4.add_subplot(111)
    sns.barplot(x=categorie_x, y=valori_y_perc, hue=categorie_x, ax=ax4, palette="plasma", legend=False)
    ax4.set_title(f"Percentuale Combinazioni (su {NUM_TEST} Domande Totali)")
    ...
    ax4.grid(axis='y', which='major', linestyle='-',  linewidth=0.8, alpha=0.7)
    ax4.grid(axis='y', which='minor', linestyle='--', alpha=0.3)
